...
<x-admin-layout :title="$title">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('admin.products.create') }}"
            class="inline-flex items-center gap-2 px-5 py-2.5 bg-black text-white text-xs font-bold uppercase tracking-widest hover:bg-indigo-600 transition-colors shadow-sm">
    ...
    <div class="bg-white border border-gray-100 shadow-sm overflow-hidden">
        @if ($products->isEmpty())
            <div class="py-20 text-center">
                ...
                <a href="{{ route('admin.products.create') }}" class="inline-block mt-4 px-6 py-2.5 bg-black text-white text-xs font-bold uppercase tracking-widest hover:bg-indigo-600 transition-colors">
                    Tambah Produk
                </a>
            </div>
        @else
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <div class="flex items-center gap-3">
                                        @if ($product->image)
                                            <img src="{{ $product->image }}" alt="{{ $product->name }}"
                                                class="w-10 h-10 object-cover rounded-sm flex-shrink-0 bg-gray-100">
                                        @else
                                            <div class="w-10 h-10 bg-gray-100 rounded-sm flex-shrink-0 flex items-center justify-center">
                                        ...
                                        <div>
                                            ...
                                            @if ($product->badge)
                                                <span class="inline-block mt-0.5 px-1.5 py-0.5 bg-indigo-50 text-indigo-700 text-[9px] font-bold uppercase tracking-wider">{{ $product->badge }}</span>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    @if ($product->is_active)
                                        <span class="px-2.5 py-1 bg-green-50 text-green-700 border border-green-200 text-[10px] font-bold uppercase tracking-wider">Aktif</span>
                                    @else
                                        <span class="px-2.5 py-1 bg-gray-50 text-gray-500 border border-gray-200 text-[10px] font-bold uppercase tracking-wider">Nonaktif</span>
                                    @endif
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.products.edit', $product) }}"
                                            class="px-3 py-1.5 border border-gray-200 text-xs font-semibold text-gray-700 hover:border-indigo-500 hover:text-indigo-600 transition-colors">
                                            Edit
                                        </a>
                                        <form method="POST" action="{{ route('admin.products.destroy', $product) }}"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="px-3 py-1.5 border border-gray-200 text-xs font-semibold text-red-600 hover:bg-red-50 hover:border-red-300 transition-colors">
                                                Hapus
                                            </button>
                                        </form>
                                    </div>
